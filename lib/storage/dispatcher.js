...
module.exports = function dispatcher (opts = {}) {
  ...
  // `onEvict` cannot cross the worker boundary (functions are not cloneable),
  // so workers only learn whether a callback exists via `hasOnEvict` and emit
  // `{ type: 'evict', ... }` messages that the main thread turns into calls.
  const workerData = { type, TTL, norm, max, sweepInterval, hasOnEvict: !!onEvict }

  const concurrencyy = concurrency ? worker : noWorker
  const setupedWorker = () => concurrencyy(workerData, onEvict)
